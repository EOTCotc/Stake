...
async function Staking(val, that) {
	try {
		...
		setTimeout(function () {
			...
			that.pending = parseFloat(Number(that.xEOTC * localStorage.getItem('yield2')).toFixed(6));
...
export const RegularUnstaking = async function (id, zq) {
	try {
		let mytron = await window.tronWeb.contract().at(regular1);
		//  valmes = distsmes1('等待区块打包确认，打包期间请不要关闭或刷新该页面');
...
//定期赎回 二期
// async function RegularUnstaking(val, id, zq, that, index) {
export const RegularUnstakingTwo = async function (id, zq) {
	try {
...
//二次质押
export const SecondPledge = async function (id, zq, zong, index) {
	try {
		let mytron = await window.tronWeb.contract().at(regular);
		//  valmes = distsmes1('等待区块打包确认，打包期间请不要关闭或刷新该页面');
		...
		setTimeout(async function () {
			...
			// that.list.splice(index, 1);
			Vue.$toast.success('质押成功！');
			StakingEotc({ num: zong, zq: zq, hx: res });
			// StakingEotc({ num: that.zynum, zq: that.nowaday, hx: res });
			// }
		}, 1000);
	} catch (e) {
	...
	mytron.get_arp().call(
		...
		function (error, result) {
			if (!error) {
				...
				that.pending = parseFloat(Number(localStorage.getItem('myXEOTCNum') * arp[1]).toFixed(6));
			} else {
				Vue.$toast.error(error);
			}
		}
	...
	if (window.tronWeb) {
		...
		var obj = setInterval(async () => {
			if (window.tronWeb.defaultAddress.base58) {
				...
				if (localStorage.getItem('walletAddress') != walletAddress || localStorage.getItem('signPledge') == null) {
